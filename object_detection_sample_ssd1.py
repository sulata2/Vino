...
def write_json(output_dictionary, filename):
...
def main():
    ...
    class_map = {1:'inv_num', 2:'inv_dt', 3:'inv_tot', 4:'stotal', 5:'vendor', 6:'addr', 7:'tax'}
    ...
    print("Image tensor shape", data["image_tensor"].shape)
    print("Image tensor type", type(data["image_tensor"]))
    print("Image tensor", data["image_tensor"])
    print("Image info", data["image_info"])

    def perform_inference(threshold_value, data, iw, ih, args):
        ...
        output_dictionary["num_detections"] = len(data)
        output_dictionary["detection_boxes"] = []
        output_dictionary["detection_scores"] = []
        output_dictionary["detection_classes"] = []
        output_dictionary["image_size"] = [iw, ih]
        ...
        for number, proposal in enumerate(data):
            ...
            if proposal[2] > 0:
                ...
                proposals_json = [i.astype(float) for i in [proposal[3], proposal[4], proposal[5], proposal[6]]]
                output_dictionary["detection_boxes"].append(proposals_json)
                output_dictionary["detection_scores"].append(confidence.astype(float))
                ...
                if proposal[2] > threshold_value:
                    ...
                    if not imid in confidences.keys():
                        confidences[imid] = []
                    confidences[imid].append(confidence)
        ...
        for cl in classes[0]:    
            output_dictionary["detection_classes"].append(cl)

        filename_json = f"out_{threshold_value}.json"
        filename_bmp = f"out_{threshold_value}.bmp"
...
(64, 64, 64)
        ...
        for imid in classes:
            ...
            for index , (box, color) in enumerate(zip(boxes[imid], itertools.cycle(colors))):
                label_confidence = "{} {:.2f}".format(class_map[classes[imid][index]], confidences[imid][index] * 100)
                print(label_confidence)
                cv2.rectangle(tmp_image, (box[0], box[1]), (box[2], box[3]), color, 2)
                (test_width, text_height), baseline = cv2.getTextSize(label_confidence,cv2.FONT_HERSHEY_SIMPLEX,0.5, 1)
                img = cv2.rectangle(tmp_image,(box[0], box[1]),(box[0] + test_width, box[1] - text_height - baseline),(255, 255, 255),thickness=cv2.FILLED)
                cv2.putText(tmp_image, label_confidence, (box[0], box[1]- baseline), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), thickness=1)       

            cv2.imwrite(filename_bmp, tmp_image)
            log.info("Image out.bmp created!")
        # -----------------------------------------------------------------------------------------------------

        write_json(output_dictionary, filename_json)
    ...
    for threshold_value in threshold_values:
        perform_inference(threshold_value, data, iw, ih, args)
